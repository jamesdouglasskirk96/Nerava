...
def verify_visit(
    ...
    try:
        ...
        # Get cluster_id if available
        cluster_id = None
        if merchant:
            from app.models.while_you_charge import ChargerMerchant, Charger
            charger_merchant = db.query(ChargerMerchant).filter(
                ChargerMerchant.merchant_id == request.merchant_id
            ).first()
            if charger_merchant:
                charger = db.query(Charger).filter(Charger.id == charger_merchant.charger_id).first()
                if charger:
                    from app.models.while_you_charge import ChargerCluster
                    cluster = db.query(ChargerCluster).filter(ChargerCluster.charger_id == charger.id).first()
                    if cluster:
