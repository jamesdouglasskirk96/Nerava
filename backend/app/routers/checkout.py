...
from ..models.while_you_charge import Merchant, ChargerMerchant, ChargerCluster, Charger
...
def resolve_cluster_from_merchant(db: Session, merchant: DomainMerchant) -> Optional[str]:
    ...
    if merchant.zone_slug:
        cluster = db.query(ChargerCluster).filter(
            ChargerCluster.name == merchant.zone_slug
        ).first()
        if cluster:
            return str(cluster.id)
    
    # Strategy 2: Find Merchant by name or location match
    merchant_record = db.query(Merchant).filter(
    ...
    if merchant_record:
        ...
        charger_link = db.query(ChargerMerchant).filter(
            ChargerMerchant.merchant_id == merchant_record.id
        ).first()
        
        if charger_link:
            # Find ChargerCluster for this Charger
            cluster = db.query(ChargerCluster).filter(
                ChargerCluster.charger_id == charger_link.charger_id
            ).first()
...
async def checkout_qr(
    ...
    if cluster_id:
        ...
        logger.warning(f"Could not resolve cluster_id for merchant {merchant.id}, routing to app without cluster")
        route = "/app/party"
