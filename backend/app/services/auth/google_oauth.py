...
class GoogleOAuthService:
            ...
            async with httpx.AsyncClient() as client:
                ...
                for loc in locations:
                    formatted_locations.append({
                        "location_id": loc.get("name", "").split("/")[-1] if loc.get("name") else "",
                        "name": loc.get("title", ""),
                        "address": loc.get("storefrontAddress", {}).get("addressLines", [""])[0] if loc.get("storefrontAddress") else "",
                        "place_id": loc.get("storeCode", ""),  # May need to map to actual place_id
                    })
    ...
    @staticmethod
    async def verify_and_check_gbp(id_token: str, access_token: Optional[str] = None) -> tuple[Dict[str, Any], List[Dict[str, str]]]:
        """
