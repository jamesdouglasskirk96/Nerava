...
def get_square_creds() -> SquareCreds:
    ...
    if is_prod:
        ...
        app_secret = settings.square_application_secret_production or settings.square_application_secret
        ...
        app_secret = settings.square_application_secret_sandbox or settings.square_application_secret
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                if orders:
                    ...
                    total_cents = sum(
                        int(order.get("total_money", {}).get("amount", 0))
                        for order in orders
                    )
            ...
            logger.info(f"No orders found or API call failed, using default AOV")
