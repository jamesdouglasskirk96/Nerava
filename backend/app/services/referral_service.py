...
def get_or_create_code(db: Session, user_id: int) -> ReferralCode:
...
def redeem_referral(db: Session, code: str, new_user_id: int) -> Optional[ReferralRedemption]:
    ...
    existing = db.query(ReferralRedemption).filter(
        ReferralRedemption.referred_user_id == new_user_id
    ).first()
    if existing:
    ...
    logger.info(f"Referral redeemed: code={code}, referrer={ref_code.user_id}, referred={new_user_id}")
    return redemption
...
def grant_referral_rewards(db: Session, user_id: int) -> bool:
    ...
    try:
        ...
        credit_wallet(db, ref_code.user_id, REFERRAL_REWARD_CENTS, "Referral reward - friend joined")
        credit_wallet(db, user_id, REFERRAL_REWARD_CENTS, "Referral welcome bonus")
        redemption.reward_granted = True
        db.commit()
        logger.info(f"Referral rewards granted: referrer={ref_code.user_id}, referred={user_id}, amount={REFERRAL_REWARD_CENTS}")
        return True
...
def get_referral_stats(db: Session, user_id: int) -> dict:
    ...
    total = db.query(func.count(ReferralRedemption.id)).filter(
        ReferralRedemption.referral_code_id == ref_code.id
    ).scalar() or 0

    granted = db.query(func.count(ReferralRedemption.id)).filter(
        ReferralRedemption.referral_code_id == ref_code.id,
