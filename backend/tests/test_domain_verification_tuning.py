...
def test_dwell_penalty(db: Session, test_user_id, seeded_domain_charger):
    ...
    from app.services.verify_dwell import ping, start_session
    from app.domains.domain_verification import DOMAIN_DWELL_OPTIMAL_S, DOMAIN_DWELL_REQUIRED_S
...
def test_debug_endpoint(db: Session, test_user_id, seeded_domain_charger):
    ...
    from app.services.verify_dwell import start_session, ping
    ...
    response = client.get(
        f"/v1/pilot/debug/session/{session_id}",
        headers={"X-Debug-Token": "domain-pilot-2024"}
    )
    ...
    if response.status_code == 404:
        ...
        session_check = db.execute(text("SELECT id FROM sessions WHERE id=:sid"), {"sid": session_id}).first()
