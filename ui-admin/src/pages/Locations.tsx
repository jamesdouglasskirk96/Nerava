...
export default function Locations() {
  ...
  const loadCandidates = async (merchantId: string) => {
    setLoading(true)
    try {
      const response = await fetch(`/v1/admin/locations/${merchantId}/google-place/candidates`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('admin_token') || ''}`
        }
      })
  ...
  const handleResolve = async (placeId: string) => {
    ...
    setResolving(true)
    try {
      const response = await fetch(`/v1/admin/locations/${selectedMerchant.id}/google-place/resolve`, {
        method: 'POST',
        headers: {
