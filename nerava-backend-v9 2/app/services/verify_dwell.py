...
def _get_session_hub_id(db: Session, session_id: str) -> Optional[str]:
    ...
    try:
        ...
        try:
            result = db.execute(text("SELECT meta FROM sessions WHERE id=:sid"), {"sid": session_id}).first()
            if result and result[0]:
                import json
        ...
        result = db.execute(text("SELECT target_id FROM sessions WHERE id=:sid"), {"sid": session_id}).first()
        if result and result[0]:
...
def _choose_target(db: Session, lat: float, lng: float, event_id: Optional[int], hub_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    if event_id:
    ...
    try:
        ...
        try:
            ...
            db.execute(text("""
                ...
                WHERE id = :sid
            """), {"meta": json.dumps(meta) if isinstance(meta, dict) else str(meta), "sid": session_id})
        except Exception:
...
def _calculate_drift_penalty(db: Session, session_id: str, lat: float, lng: float, ts: datetime) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            result = db.execute(text("SELECT last_lat, last_lng, updated_at FROM sessions WHERE id=:sid"), {"sid": session_id}).first()
            ...
            for ping in ping_history:
                ping_ts = datetime.fromisoformat(ping["ts"]) if isinstance(ping["ts"], str) else ping["ts"]
...
def _calculate_verification_score(
    ...
    if distance_m > radius_m:
        ...
        distance_penalty = min(MAX_DISTANCE_PENALTY, int(excess_distance * SCORE_DISTANCE_PENALTY_PER_M))
    ...
    if accuracy_m > min_accuracy_m:
        ...
        accuracy_penalty = min(MAX_ACCURACY_PENALTY, int(excess_accuracy * SCORE_ACCURACY_PENALTY_PER_M))
...
def ping(db: Session, *, session_id: str, lat: float, lng: float, accuracy_m: float, ts: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    if accuracy_m > min_acc:
        ...
        return {
            "ok": True, "verified": False, "reason": "accuracy", 
            "accuracy_m": accuracy_m, "min_accuracy_m": min_acc, 
    ...
    drift_result = _calculate_drift_penalty(db, session_id, lat, lng, now) if hub_id == "domain" else {"penalty": 0, "drift_m": 0}
    ...
    if not target:
        ...
        if sel:
            try:
                ...
                """), {"tt": sel["target_type"], "ti": sel["target_id"], "tn": sel["target_name"], "rm": int(radius_m), "sid": session_id})
                db.commit()
    ...
    """), {
        "llat": lat, "llng": lng, "acc": accuracy_m, "sid": session_id,
        "dwell": new_dwell, "req": dwell_required_s,
    })
    db.commit()
    ...
    if new_dwell >= dwell_required_s:
        ...
        response = {
            "ok": True, "verified": True, "rewarded": rewarded, 
            "reward_cents": int(getattr(settings,'verify_reward_cents',200)), 
            "wallet_delta_cents": wallet_delta, "pool_delta_cents": pool_delta, 
            "dwell_seconds": new_dwell, "ping_count": int((row.get("ping_count") or 0) + 1),
