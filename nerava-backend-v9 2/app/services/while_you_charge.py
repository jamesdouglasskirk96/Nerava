...
async def find_and_link_merchants(
    ...
    # Use subquery to avoid JSON comparison issues with place_types column in DISTINCT
    ...
    base_query = """
        ...
        FROM merchants m
        JOIN charger_merchants cm ON m.id = cm.merchant_id
        WHERE cm.charger_id = ANY(:charger_ids)
        AND cm.walk_duration_s <= :max_walk_seconds
    """
    ...
    if category:
        base_query += " AND m.category = :category"
        params["category"] = category
    elif merchant_name:
        base_query += " AND m.name ILIKE :merchant_name"
        params["merchant_name"] = f"%{merchant_name}%"
    
    merchant_ids_query = text(base_query)
    result = db.execute(merchant_ids_query, params)
    merchant_ids = [row[0] for row in result.fetchall()]
    
    # Now query merchants by IDs (avoids JSON comparison in DISTINCT)
    if merchant_ids:
        existing_merchants = db.query(Merchant).filter(Merchant.id.in_(merchant_ids)).all()
    else:
        existing_merchants = []
