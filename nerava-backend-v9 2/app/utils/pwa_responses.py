...
def shape_merchant(merchant: Dict[str, Any], user_lat: Optional[float] = None, user_lng: Optional[float] = None) -> Dict[str, Any]:
    ...
    # 3. Static file paths for known merchants (Asadas Grill)
    # 4. icon (from Google Places icon) - fallback
    logo_url = None
    
    # Check for known merchants with static photos (before checking photo_url)
    merchant_name = str(merchant.get("name", "")).lower()
    if "asadas" in merchant_name and "grill" in merchant_name:
        # Asadas Grill has static photos - use relative path so frontend can prepend API base URL
        logo_url = "/static/merchant_photos_asadas_grill/asadas_grill_01.jpg"
    ...
    if not logo_url and "photo_url" in merchant and merchant.get("photo_url"):
        ...
        if photo_ref and str(photo_ref).strip():
            ...
            if photo_ref_str.startswith("http"):
                # Already a full URL - extract relative path if it's our static files
                if "/static/merchant_photos_asadas_grill/" in photo_ref_str:
                    # Extract relative path from full URL
                    logo_url = "/static/merchant_photos_asadas_grill/" + photo_ref_str.split("/static/merchant_photos_asadas_grill/")[-1]
                else:
    ...
    # Include logo_url and photo_url if we have any URL
    # Both fields are included for frontend compatibility (some components use photo_url, others use logo_url)
    if logo_url and logo_url.strip():
        result["logo_url"] = logo_url.strip()
        result["photo_url"] = logo_url.strip()  # Alias for frontend compatibility
