...
# Initialize PostHog analytics (optional server-side)
from app.integrations.posthog import init_posthog
init_posthog()
...
# Initialize validation tracking variables before try block
...
from .routers import gpt, meta, sessions, stripe_api, purchase_webhooks, dev_tools, merchant_api, merchant_ui, internal
...
from .routers import intent, vehicle_onboarding, perks, merchant_onboarding, merchants
...
# Note: _startup_validation_failed and _startup_validation_errors are initialized earlier (before validation try block)
...
# Try multiple possible locations (parent directory or same directory)
MERCHANT_PHOTOS_DIR = None
for possible_path in [
    Path(__file__).parent.parent / "merchant_photos_asadas_grill",  # Same level as app/
    Path(__file__).parent.parent.parent / "merchant_photos_asadas_grill",  # Parent of backend/
]:
    if possible_path.exists() and possible_path.is_dir():
        MERCHANT_PHOTOS_DIR = possible_path
        break

if MERCHANT_PHOTOS_DIR:
    ...
    logger.warning("merchant_photos_asadas_grill directory not found - photos will not be served")

# Mount Google Places merchant photos directory
GOOGLE_PHOTOS_DIR = Path(__file__).parent.parent / "static" / "merchant_photos_google"
if GOOGLE_PHOTOS_DIR.exists() and GOOGLE_PHOTOS_DIR.is_dir():
    app.mount("/static/merchant_photos_google", StaticFiles(directory=str(GOOGLE_PHOTOS_DIR), html=False), name="merchant_photos_google")
    logger.info("Mounted /static/merchant_photos_google from directory: %s", str(GOOGLE_PHOTOS_DIR))
...
app.include_router(sms.router, prefix="/v1/sms", tags=["sms"])  # /v1/sms/*
