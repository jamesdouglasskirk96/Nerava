...
from sqlalchemy import Column, Integer, String, Float, Boolean, DateTime, Time, ForeignKey, Text, Index
...
class Merchant(Base):
    ...
    user_rating_count = Column(Integer, nullable=True)  # Number of reviews
    price_level = Column(Integer, nullable=True)  # 1-4 (Google Places)
    phone = Column(String, nullable=True)
    website = Column(String, nullable=True)
    description = Column(Text, nullable=True)  # Business description
    
    # Opening hours (stored as JSON)
    opening_hours = Column(JSON, nullable=True)  # { "open_now": bool, "weekday_text": [...], "periods": [...] }
    
    # Google Places types (array)
    place_types = Column(JSON, default=list)
    place_id = Column(String, nullable=True, index=True)  # Google Places place_id
...
class MerchantPerk(Base):
    ...
    # Cap and time window fields for exclusives
    daily_cap = Column(Integer, nullable=True)  # Max activations per day
    start_time = Column(Time, nullable=True)  # Start time for time window
    end_time = Column(Time, nullable=True)  # End time for time window
    days_of_week = Column(String(20), nullable=True)  # "1,2,3,4,5" for Mon-Fri (1=Monday)
...
class MerchantFavorite(Base):
    ...
    __table_args__ = (
        Index('idx_merchant_favorite_unique', 'user_id', 'merchant_id', unique=True),
    )
