...
class TestFindChargersNear:
    ...
    async def test_find_chargers_near_fetches_from_api(self, db: Session):
        ...
        with patch('app.services.while_you_charge.fetch_chargers_in_bbox', new_callable=AsyncMock) as mock_fetch:
...
class TestFindAndLinkMerchants:
    ...
    async def test_find_and_link_merchants_fetches_from_google(self, db: Session):
        ...
        with patch('app.services.while_you_charge.search_places_near', new_callable=AsyncMock) as mock_search, \
             patch('app.services.while_you_charge.get_walk_times', new_callable=AsyncMock) as mock_walk, \
             patch('app.services.while_you_charge.get_place_details', new_callable=AsyncMock) as mock_details:
