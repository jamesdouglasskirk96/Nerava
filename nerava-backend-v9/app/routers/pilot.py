...
def start_session(
    ...
    for charger_config in DOMAIN_CHARGERS:
        ...
        if distance < min_distance:
            ...
            nearest_charger_config = charger_config
    
    # Shape charger for PWA
    if nearest_charger_config:
        nearest_charger = shape_charger(
            {
                "id": nearest_charger_config["id"],
                "name": nearest_charger_config["name"],
                "lat": nearest_charger_config["lat"],
                "lng": nearest_charger_config["lng"],
                "network_name": nearest_charger_config.get("network_name"),
            },
    ...
    try:
        ...
        return {
            ...
            "dwell_required_s": normalize_number(result.get("dwell_required_s", 90)),
    ...
    try:
        ...
        if not is_verified:
            ...
            if "distance_m" in result:
                response["distance_m"] = normalize_number(result["distance_m"])
            if "radius_m" in result:
                response["radius_m"] = normalize_number(result["radius_m"])
...
def get_activity(
    ...
    # Get sessions (simplified for PWA)
