...
def test_user_register():
    r = client.post(f"{BASE}/v1/users/register", json={"email": USER, "name": "Demo User"})
    assert r.status_code in (200, 201), r.text
    assert "email" in r.json()

def test_user_prefs():
    prefs_payload = {"pref_coffee": True, "pref_food": True}
    r = client.post(f"{BASE}/v1/users/{USER}/prefs", json=prefs_payload)
    assert r.status_code == 200, r.text
    r2 = client.get(f"{BASE}/v1/users/{USER}/prefs")
    assert r2.status_code == 200, r2.text
    j = r2.json()
    assert j.get("pref_coffee") is True

def test_wallet_credit_debit():
    r_before = client.get(f"{BASE}/v1/wallet", params={"user_id": USER})
    assert r_before.status_code == 200, r_before.text
    before = r_before.json().get("balance_cents", 0)

    r_credit = client.post(f"{BASE}/v1/wallet/credit_qs", params={"user_id": USER, "cents": 500})
    assert r_credit.status_code == 200, r_credit.text

    r_after = client.get(f"{BASE}/v1/wallet", params={"user_id": USER})
    after = r_after.json().get("balance_cents", 0)
    assert after >= before + 500

@pytest.fixture(scope="module")
def local_merchant_and_perk():
    ...
    r_m = client.post(f"{BASE}/v1/local/merchant", json=m_payload)
    ...
    r_p = client.post(f"{BASE}/v1/local/perk", json=p_payload)
...
def test_unified_merchants_includes_local(local_merchant_and_perk):
    ...
    r = client.get(f"{BASE}/v1/merchants/nearby", params=params)
    assert r.status_code == 200, r.text
    items = r.json()
...
def test_perk_claim_idempotent_and_wallet(local_merchant_and_perk):
    pid = local_merchant_and_perk["perk_id"]

    # balance before
    before = client.get(f"{BASE}/v1/wallet", params={"user_id": USER}).json().get("balance_cents", 0)

    # first claim -> should credit (newly_claimed true)
    r1 = client.post(f"{BASE}/v1/local/perk/claim", json={"perk_id": pid, "user_id": USER})
    ...
    mid = client.get(f"{BASE}/v1/wallet", params={"user_id": USER}).json().get("balance_cents", 0)
    assert mid >= before + 75  # awarded 75¢

    # second claim -> should be idempotent (no extra credit)
    r2 = client.post(f"{BASE}/v1/local/perk/claim", json={"perk_id": pid, "user_id": USER})
    assert r2.status_code == 200, r2.text
    j2 = r2.json()
    assert j2.get("idempotent") is True
    after = client.get(f"{BASE}/v1/wallet", params={"user_id": USER}).json().get("balance_cents", 0)
    assert after == mid  # unchanged on duplicate claim

def test_reservation_and_hubs():
    hubs = client.get(f"{BASE}/v1/hubs/nearby", params={"lat": LAT, "lng": LNG, "radius_km": 2}).json()
    assert isinstance(hubs, list)
    if hubs:
        hub_id = hubs[0].get("id", "hub_domain_A")
        start = (datetime.utcnow() + timedelta(minutes=5)).replace(microsecond=0).isoformat() + "Z"
        r = client.post(f"{BASE}/v1/reservations/soft", json={"hub_id": hub_id, "user_id": USER, "start_iso": start, "minutes": 30})
        assert r.status_code == 200, r.text
