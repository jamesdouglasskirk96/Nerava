import { Wifi, ThumbsUp, ThumbsDown, Bath } from 'lucide-react'
      ...
      {amenities.map(({ key, icon: Icon, label, data }) => (
        <button
          ...
          className={`flex flex-col items-center gap-1 ${
            !interactive && onAmenityClick
              ? 'cursor-pointer hover:opacity-80 active:scale-95 transition-all'
              : ''
          }`}
          aria-label={`${label} amenity votes`}
        >
          {/* Icon with color based on net votes */}
          <Icon className={`w-6 h-6 ${getIconColor(data)}`} />
          ...
          {interactive && (
            <div className="flex flex-col gap-0.5">
              ...
              <button
                ...
                aria-label={`Vote down for ${label}`}
              >
