...
export default function Users() {
  ...
  const searchUsers = async () => {
    setLoading(true)
    try {
      const response = await fetch(`/api/v1/admin/users?query=${encodeURIComponent(searchQuery)}`, {
        headers: {
  ...
  const handleAdjustWallet = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/v1/admin/users/${selectedUser.id}/wallet/adjust`, {
        method: 'POST',
        headers: {
