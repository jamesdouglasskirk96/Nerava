...
import { Zap, Car, User, ArrowLeft, Share2, CheckCircle, ThumbsUp, ThumbsDown } from "lucide-react";
import { useState, useEffect } from "react";

// Nerava - Moment of Charge Discovery Network
...
export default function App() {
  ...
  const [exclusiveRemainingTime, setExclusiveRemainingTime] = useState(45);
  ...
  const handleActivateExclusive = (merchant: Merchant) => {
    // Go directly to refuel intent modal - no authentication required
  ...
  const handleArrived = () => {
    // Skip the arrival modal - driver has already shown the code
    ...
    <>
      ...
      {(!activeExclusive || !showActiveExclusiveView) && (
        <div className="min-h-screen bg-white text-[#050505] max-w-md mx-auto flex flex-col h-screen overflow-hidden">
          ...
          <header className="bg-white border-b border-[#E4E6EB] flex-shrink-0">
            <div className="flex items-center justify-between px-5 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <Car className="w-5 h-5 text-[#050505]" />
            ...
            {activeExclusive && !showActiveExclusiveView && (
              <button
                onClick={() => setShowActiveExclusiveView(true)}
                className="w-full bg-[#1877F2] text-white py-2.5 px-5 flex items-center justify-between hover:bg-[#166FE5] active:scale-[0.99] transition-all"
              >
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-white rounded-full animate-pulse" />
                  <span className="text-sm font-medium">{activeExclusive.name}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs">{exclusiveRemainingTime} min left</span>
          ...
          <div className="text-center px-6 pt-4 pb-2 flex-shrink-0">
            <h1 className="text-2xl sm:text-3xl mb-1 whitespace-nowrap overflow-hidden text-ellipsis">
              {currentLayer === 1 && `${chargers.filter(c => c.availableStalls > 0).length} chargers with open stalls`}
              {currentLayer === 2 && selectedCharger && (
                <div className="whitespace-normal">
                  <div>Secure your spot</div>
                  <div>while you charge</div>
                </div>
              )}
              {currentLayer === 3 && ""}
            </h1>
      ...
      {showCompletionModal && activeExclusive && (
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-[60] p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full mb-8 shadow-2xl">
            ...
            <h2 className="text-2xl text-center mb-3">Reservation Completed</h2>
      ...
      {showAccount && (
        <AccountPage
          ...
          likedMerchantsCount={likedMerchants.size}
          onLogout={() => {
            setIsAuthenticated(false);
            setActiveExclusive(null);
            setShowAccount(false);
            localStorage.removeItem('neravaAuth');
